      ...
      async function updateDisplay(value) {
        try {
          if (value === '+' || value === '-' || value === '*' || value === '/') {
            ...
            if (this.operator === '+') {
              const response = await axios.get(`http://localhost:8080/calculator/add/${this.num1}/${this.num2}`);
              console.log(response);
              console.log(response.data);
              this.num1 = response.data.result;
            }
            if (this.operator === '-') {
              const response = await axios.get(`http://localhost:8080/calculator/subtract/${this.num1}/${this.num2}`);
              console.log(response);
              console.log(response.data);
              this.num1 = response.data.result;
            }
            if (this.operator === '*') {
              const response = await axios.get(`http://localhost:8080/calculator/multiply/${this.num1}/${this.num2}`);
              console.log(response);
              console.log(response.data);
              this.num1 = response.data.result;
            }
            if (this.operator === '/') {
              const response = await axios.get(`http://localhost:8080/calculator/divide/${this.num1}/${this.num2}`);
              console.log(response);
              console.log(response.data);
          ...
          if (typeof this.operator === 'undefined' || this.operator === 'undefined') {
            display.value = this.num1;
          }

          else if (typeof this.num2 === 'undefined' || this.num2 === 'undefined') {
            display.value = this.num1 + ' ' + this.operator;
          }

          else {
            display.value = this.num1 + ' ' + this.operator + ' ' + this.num2;
          }
        } catch (error) {
